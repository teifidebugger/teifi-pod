...
export function ScheduleHeader({ dates, today, mode }: ScheduleHeaderProps) {
    const { range, valueToPixels } = useTimelineContext()
    const todayMs = new Date(today + "T00:00:00").getTime()
    const todayInRange = todayMs >= range.start && todayMs < range.end

    // Month mode: render one column per calendar month
    if (mode === "month") {
        // Find first day of each month within range
        const months: string[] = []
        ...
        return (
            <div className="flex border-b border-sidebar-border bg-sidebar sticky top-[102px] z-20">
                <div className="flex-shrink-0 px-4 py-3 border-r border-sidebar-border flex items-center" style={{ width: SIDEBAR_WIDTH }}>
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Team Member</span>
                </div>
    ...
    // Week mode: 2-row header — week range on top, individual day numbers on bottom
    if (mode === "week") {
    ...
    return (
        <div className="flex border-b border-sidebar-border bg-sidebar sticky top-[102px] z-20">
            <div
                className="flex-shrink-0 px-4 py-3 border-r border-sidebar-border flex items-center"
